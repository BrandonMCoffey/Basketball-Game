...
public class CatalogManager : MonoBehaviour
{
    ...
    [SerializeField] private CatalogCard _cardPrefab;
    [SerializeField] private TMP_Text _cardsActiveCount;
    
    private List<CatalogCard> _cards;

    private void Start()
    {
        InitializeCards(GameManager.Instance);
        UpdateCardsActiveCount();
        GameManager.UpdateActivePlayers += UpdateCardsActiveCount;
    }

    public void StartGame() => GameManager.Instance.StartGame();

    private void UpdateCardsActiveCount()
    {
        _cardsActiveCount.text = $"Players Active: {GameManager.Instance.CardActiveCount}";
    }

    private void InitializeCards(GameManager instance)
    {
        _cards = new List<CatalogCard>(instance.Players.Count);
